/**
 *  Sourced from https://github.com/kennelbound-smartthings/kevo-unofficial/blob/master/devicetypes/kennelbound-smartthings/kevolock/kevo-unofficial.src/kevo-unofficial.groovy
 *  Updates:
 *  02-23-2019 : Doing my own things with removing refresh and changing colors to match
 */
...
metadata {
    ...
    definition(name: "Kevo Lock", namespace: "SmartThings", author: "Alvin Chen") {
        capability "Actuator"
        capability "Sensor"
    ...
    tiles(scale: 2) {
        multiAttributeTile(name: "toggle", type: "generic", width: 6, height: 4) {
            tileAttribute("device.lock", key: "PRIMARY_CONTROL") {
                attributeState "locked", label: 'Locked', action: "lock.unlock", icon: "st.locks.lock.locked", backgroundColor: "#F5A623", nextState: "unlocking"
                attributeState "locking", label: 'Locking', icon: "st.locks.lock.locked", backgroundColor: "#F5A623", nextState: "locked"
                ...
                attributeState "unlocked", label: 'Unlocked', action: "lock.lock", icon: "st.locks.lock.unlocked", backgroundColor: "#70B844", nextState: "locking"
                attributeState "unlocking", label: 'Unlocking', icon: "st.locks.lock.unlocked", backgroundColor: "#70B844", nextState: "unlocked"
...
def updated() {
    log.trace "updated()"
    sendEvent(name: "checkInterval", value: 10 * 60, displayed: false, data: [protocol: "cloud", scheme: "untracked"])
    refresh()
}
